...
import { sandboxes, notifications, generateId } from "@/lib/storage";
import type { AdvisorNote, Notification } from "@/lib/types";
...
export async function POST(
  ...
  // Send notification to the sandbox owner (client)
  const clientId = sb.userId;
  if (clientId && clientId !== DEV_ADVISOR_ID) {
    const notif: Notification = {
      id: generateId("notif"),
      userId: clientId,
      type: "advisor_note",
      title: "New advisor note",
      body: `${note.advisorName} left a note on "${sb.name}": "${note.text.slice(0, 100)}${note.text.length > 100 ? "…" : ""}"`,
      sandboxId: id,
      read: false,
      createdAt: new Date().toISOString(),
    };
