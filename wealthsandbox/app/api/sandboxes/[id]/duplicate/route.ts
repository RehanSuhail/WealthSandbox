...
export async function POST(
  ...
  try {
    ...
    const newSandbox = sandboxes.create({
      ...
      // Carry over risk analysis & metadata from original
      ...(original.healthScore ? { healthScore: original.healthScore } : {}),
      ...(original.riskMetrics ? { riskMetrics: original.riskMetrics } : {}),
      ...(original.lastAnalyzedAt ? { lastAnalyzedAt: original.lastAnalyzedAt } : {}),
